...
export default function WelcomePage() {
  ...
  useEffect(() => {
    ...
    if (stored) { setData(JSON.parse(stored)); setLoading(false); localStorage.removeItem("welcomeData"); return; }
    if (leadId) { fetch(`/api/lead-status/${leadId}`).then(res => res.json()).then(data => { setData(data); setLoading(false); }).catch(() => setLoading(false)); } 
    else { setLoading(false); }
  }, [leadId]);

  if (loading) return <div className="min-h-screen bg-gray-900 flex items-center justify-center"><div className="text-white text-xl">Loading your story insights...</div></div>;
  if (!data) return <div className="min-h-screen bg-gray-900 flex items-center justify-center"><div className="text-white text-xl">No data found. Please go back and submit your story.</div></div>;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-black flex items-center justify-center p-4">
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 max-w-3xl w-full border border-white/20 shadow-2xl">
        <div className="text-center mb-8"><h1 className="text-4xl font-bold text-white">Omega <span className="text-yellow-400">Forge</span></h1><p className="text-gray-400 mt-1">Forging Stories Into Growth Engines</p></div>
        <div className="text-center mb-8"><h2 className="text-3xl md:text-4xl font-bold text-white">We already get you, <span className="text-yellow-400">{data.business_name}</span>.</h2></div>
        ...
        <div className="mt-8 text-center"><p className="text-gray-300 mb-4 text-lg">Based on your story, this is exactly the direction we recommend.</p><button onClick={() => alert("✅ We'll reach out to you within 24 hours to lock in your strategy call!")} className="px-8 py-3 bg-yellow-400 hover:bg-yellow-500 text-black font-semibold rounded-lg transition-all text-lg">🚀 Approve This Direction & Book My Strategy Call</button></div>
      </div>
    </div>
  );
}